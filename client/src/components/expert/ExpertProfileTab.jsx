...
export default function ExpertProfileTab({ user, brcData }) {
  ...
  const assignedBrcs = assignedBrcCodes.length > 0 ? brcData.filter(b => assignedBrcCodes.includes(b.code)) : [];
    ...
    <>
      <div className="space-y-12 animate-fade-in-up">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {/* Right Column: Forms & Assigned BRCs */}
          <div className="lg:col-span-2 space-y-8">
            ...
            {/* Assigned BRCs Read-Only */}
            <div className="bg-white border border-on-surface/10 rounded-3xl p-8 shadow-sm">
              <h3 className="text-xl font-bold text-on-surface mb-2 flex items-center gap-2">
                <span className="material-symbols-outlined text-primary">school</span>
                Assigned STREAM Hubs
              </h3>
              <p className="text-sm text-secondary mb-6">These are the academic centers currently assigned to your portfolio by the system administrator.</p>
              
              {assignedBrcs.length === 0 ? (
                <div className="p-8 rounded-2xl bg-surface-container border border-dashed border-outline/20 text-center text-secondary">
                  <span className="material-symbols-outlined text-4xl mb-3 opacity-50">map</span>
                  <p>You have no assigned hubs.</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {assignedBrcs.map(brc => (
                    <div key={brc.code} className="p-5 rounded-2xl border border-outline/10 bg-surface-container-low flex items-start gap-4">
